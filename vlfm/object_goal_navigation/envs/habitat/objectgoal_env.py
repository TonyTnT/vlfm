...
class ObjectGoal_Env(habitat.RLEnv):
    ...
    def make_observation_space(self):
        rgb_config = self.conf.SIMULATOR.RGB_SENSOR
        depth_config = self.conf.SIMULATOR.DEPTH_SENSOR

        dim = 1 if self._no_pitch else 2
        compass_space = gym.spaces.Box(high=np.pi, low=-np.pi, shape=(dim,), dtype=np.float32)
        objectgoal_space = gym.spaces.Discrete(6)
        rgb_space = gym.spaces.Box(high=255, low=0, shape=(rgb_config.HEIGHT, rgb_config.WIDTH, 3), dtype=np.uint8)
        depth_space = gym.spaces.Box(
            high=1.0, low=0.0, shape=(depth_config.HEIGHT, depth_config.WIDTH, 1), dtype=np.float32
        )
        return gym.spaces.Dict(
            {"rgb": rgb_space, "depth": depth_space, "compass": compass_space, "objectgoal": objectgoal_space}
        )
    ...
    def generate_new_episode(self):
        ...
        while not loc_found:
            ...
            if possible_starting_locs.sum() != 0:
                ...
                print("Invalid object: {} / {} / {}".format(scene_name, floor_height, goal_name))
                possible_cats.remove(goal_idx)
                scene_info[floor_idx]["sem_map"][goal_idx + 1, :, :] = 0.0
                self.dataset_info[scene_name][floor_idx]["sem_map"][goal_idx + 1, :, :] = 0.0
    ...
    def step(self, action):
        ...
        self.info["gps"] = obs["gps"]
        self.info["heading"] = obs["heading"]
        self.info["compass"] = obs["compass"]
