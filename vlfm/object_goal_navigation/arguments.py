...
def get_args():
    ...
    parser.add_argument("--num_training_frames", type=int, default=10000000, help="total number of training frames")
    parser.add_argument("--num_eval_episodes", type=int, default=200, help="number of test episodes per scene")
    parser.add_argument(
        "--num_train_episodes",
        type=int,
        default=10000,
    ...
    parser.add_argument(
        "-d", "--dump_location", type=str, default="./tmp/", help="path to dump models and log (default: ./tmp/)"
    )
    parser.add_argument("--exp_name", type=str, default="exp1", help="experiment name (default: exp1)")
    parser.add_argument("--save_periodic", type=int, default=500000, help="Model save frequency in number of updates")
    parser.add_argument(
        "--load",
        type=str,
        default="0",
    ...
    parser.add_argument("-efw", "--env_frame_width", type=int, default=640, help="Frame width (default:640)")
    parser.add_argument("-efh", "--env_frame_height", type=int, default=480, help="Frame height (default:480)")
    parser.add_argument("-fw", "--frame_width", type=int, default=160, help="Frame width (default:160)")
    parser.add_argument("-fh", "--frame_height", type=int, default=120, help="Frame height (default:120)")
    parser.add_argument("-el", "--max_episode_length", type=int, default=500, help="""Maximum episode length""")
    parser.add_argument(
        "--task_config",
        type=str,
    ...
    parser.add_argument("--split", type=str, default="train", help="dataset split (train | val | val_mini) ")
    parser.add_argument("--camera_height", type=float, default=0.88, help="agent camera height in metres")
    ...
    parser.add_argument("--min_depth", type=float, default=0.5, help="Minimum depth for depth sensor in meters")
    parser.add_argument("--max_depth", type=float, default=5.0, help="Maximum depth for depth sensor in meters")
    parser.add_argument("--success_dist", type=float, default=1.0, help="success distance threshold in meters")
    parser.add_argument("--floor_thr", type=int, default=50, help="floor threshold in cm")
    parser.add_argument("--min_d", type=float, default=1.5, help="min distance to goal during training in meters")
    parser.add_argument("--max_d", type=float, default=100.0, help="max distance to goal during training in meters")
    ...
    parser.add_argument("--entropy_coef", type=float, default=0.001, help="entropy term coefficient (default: 0.01)")
    ...
    parser.add_argument("--num_global_steps", type=int, default=20, help="number of forward steps in A2C (default: 5)")
    parser.add_argument("--ppo_epoch", type=int, default=4, help="number of ppo epochs (default: 4)")
    parser.add_argument("--num_mini_batch", type=str, default="auto", help="number of batches for ppo (default: 32)")
    parser.add_argument("--clip_param", type=float, default=0.2, help="ppo clip parameter (default: 0.2)")
    parser.add_argument("--use_recurrent_global", type=int, default=0, help="use a recurrent global policy")
    parser.add_argument(
        "--num_local_steps",
        type=int,
        default=25,
    ...
    parser.add_argument(
        "--intrinsic_rew_coeff", type=float, default=0.02, help="intrinsic exploration reward coefficient"
    )
    parser.add_argument("--num_sem_categories", type=float, default=16)
    parser.add_argument("--sem_pred_prob_thr", type=float, default=0.9, help="Semantic prediction confidence threshold")
    ...
    if args.cuda:
        if args.auto_gpu_config:
            ...
            for i in range(num_gpus):
                gpu_memory = min(gpu_memory, torch.cuda.get_device_properties(i).total_memory / 1024 / 1024 / 1024)
                assert (
